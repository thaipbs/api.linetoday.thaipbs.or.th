...
class UpdateLineTodayXMLVideo extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'UpdateLineTodayXMLVideo:daily';
     ...
     * @var string
     */
    protected $description = 'Command for update LineToday XML Video every 10 minutes per daily';
    ...
    public function handle()
    {
		...
		$items =  Http::get($programURL)->json()['data'];
		
		$updateTime = array_column($items , 'updated_at');
		
		$items[0]['uuID'] = '02'.$items[0]['id'].strtotime(max($updateTime)).'000';
		$items[0]['uuTime'] = strtotime(max($updateTime)).'000';
		
		if(!empty($items)){
			
			$content = View::make('linetodayVideo', compact('items'))
						->withHeader([
						...
						->render();
			$destinationPath = public_path()."/xml_linetoday/video.xml";
